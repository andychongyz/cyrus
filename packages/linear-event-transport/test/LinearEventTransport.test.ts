...
describe("LinearEventTransport", () => {
	describe("published source IPs in direct mode", () => {
		...
		// Fork behavior: the IP allowlist is advisory only (Linear routes webhooks
		// through Cloudflare, so the source IP may not match the documented GCP IPs).
		// An unlisted IP is logged but still proceeds to HMAC verification, so a
		// valid signature is accepted regardless of source IP.
		it.each([
