...
Every commit message MUST end with a Co-Authored-By trailer for the bot identity.
Use the bot username from `<github_bot_username>` and user ID from `<github_bot_user_id>`
in the `<agent_context>` block of the system prompt:

```
Co-Authored-By: <bot_username> <<bot_user_id>+<bot_username>@users.noreply.github.com>
```

If `<github_bot_user_id>` is not present in the agent context, omit the trailer.
