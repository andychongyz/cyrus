...
### Fixed
- Changed Linear webhook IP validation from blocking (403) to advisory (warn-and-continue) in `LinearEventTransport`. Linear now routes webhooks through Cloudflare IPs (104.x.x.x, 172.69.x.x) that aren't in the documented GCP allowlist. HMAC signature verification remains the authoritative security control; the IP mismatch is now logged as a warning without rejecting valid requests.
- Re-inject `<context_reminder>` (with `<base_branch>`) immediately before `verify-and-ship` skill runs via a new `PreToolUse` hook in `RunnerConfigBuilder`. After many tool turns or context compression the agent no longer loses track of the correct base branch when running `gh pr create`. Hotfix sessions (source `hotfix-elicitation`) also receive the `<branch_type>hotfix</branch_type>` / `<branch_note>` block. Extracted a `buildContextReminderBlock` private helper in `EdgeWorker.ts` used by both `buildContinuationPrompt` and the new hook. Added `PreToolUseHookInput` to `cyrus-claude-runner` re-exports. 9 new TDD unit tests in `packages/edge-worker/test/verify-and-ship.context-reminder.test.ts`.
